...
class LDBSCAN:
        ...
        for i in np.arange(n):
            ...
            for j in np.arange(n):
                ...
                for k in np.arange(m):
                    distance[j] += (float(first_object[k]) - float(second_object[k])) * (
                                float(first_object[k]) - float(second_object[k]))
                distance[j] = math.sqrt(distance[j])
            sequence = self.getTopKSequenceOptimized(distance, MinPts + 1, False)
            for j in np.arange(len(sequence)):
    ...
    def calculateLRD(self, MinPts_LOF):
        ...
        for i in np.arange(len(self.records)):
            reach_dist = 0
            for j in np.arange(MinPts_LOF):
        ...
        while (unassigned_objs_id):
            ...
            if (self.lof[obj_id] < core_lof):
                ...
                while (tempList):
                    ...
                    for i in np.arange(MinPts_LDBSCAN):
                ...
                unassigned_objs_id.pop(unassigned_objs_id.index(int(obj_id)))
        print("id,", )
        for i in np.arange(len(self.records[0])):
            print("attri" + str(i + 1) + ",", )

        print("LRD,LOF,clusterID")
        for i in np.arange(len(self.belong)):
            print(str(i), ",", )
            record = self.records[i]
            for j in np.range(len(record)):
                print(record[j], ",", )
...
class Quicksort:
    ...
    def sort(self, org_array, breakRank, ascending):
        ...
        for i in np.arange(len(org_array)):
            sequence[i] = i
            array[i] = org_array[i]
